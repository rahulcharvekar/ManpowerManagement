...
const WorkerUploadFilesSummaryTable = () => {
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('fileName')}>File Name</th>
              <th className="px-4 py-2">Total Records</th>
              <th className="px-4 py-2">Uploaded</th>
              <th className="px-4 py-2">Validated</th>
              <th className="px-4 py-2">Rejected</th>
              <th className="px-4 py-2">Status</th>
              <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('uploadDate')}>Upload Date</th>
              <th className="px-4 py-2">Ready for Payment</th>
            </tr>
          </thead>
          <tbody className="bg-white divide-y divide-gray-200">
            {loading ? (
              ...
              <tr><td colSpan={8} className="text-center py-8">No files found</td></tr>
            ) : (
              data.map(row => (
                <tr key={row.fileId}>
                  ...
                  <td className="px-4 py-2">{row.uploadDate?.slice(0, 19).replace('T', ' ')}</td>
                  <td className="px-4 py-2">{row.readyForPayment ? 'Yes' : 'No'}</td>
                </tr>
              ))
            )}
          </tbody>
...
export default WorkerUploadFilesSummaryTable;
